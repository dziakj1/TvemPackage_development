...
tvem <- function(data,
  ...
  ##################################
  #####
  # The following lines involving the object m are adapted from polr 
  # in the MASS package  (version 7.3-51.1, 2018-10-31)
  # Brian Ripley, Bill Venables, Douglas M. Bates, Kurt Hornik,
  # Albrecht Gebhardt and David Firth
  # Venables, W. N. & Ripley, B. D. (2002) Modern Applied Statistics with S.
  # Fourth Edition. Springer, New York. ISBN 0-387-95457-0
  m <- match.call(expand.dots = FALSE)
  m$formula <- m$invar_effects <- m$family <- m$grid <- NULL
  ...
  #print(summary(data_for_analysis));
  #print(bam_formula);
  ...
  # peanut butter or tofu is also fine!
  ...
  standard_error_b0 <- drop(sqrt(sapply(X=1:nrow(basis_for_b0),
                                        FUN=temp_function)));
  upper_b0 <- estimated_b0 + 1.96*standard_error_b0;
  lower_b0 <- estimated_b0 - 1.96*standard_error_b0;
  ...
  grid_standard_error_b0 <- drop(sqrt(sapply(X=1:nrow(grid_basis_for_b0),
                                             FUN=temp_function)));
  grid_upper_b0 <- grid_estimated_b0 + 1.96*grid_standard_error_b0;
  grid_lower_b0 <- grid_estimated_b0 - 1.96*grid_standard_error_b0;
  ...
  if (num_varying_effects>0) {
    for (i in 1:num_varying_effects) {
      ...
      standard_error_this_b <- drop(sqrt(sapply(X=1:nrow(basis_for_b0),
                                                FUN=temp_function)));
      upper_this_b <- estimated_this_b + 1.96*standard_error_this_b;
      lower_this_b <- estimated_this_b - 1.96*standard_error_this_b;
      fitted_coefficients[[1+i]] <- data.frame(estimate = estimated_this_b,
      ...
      grid_standard_error_this_b <- drop(sqrt(sapply(X=1:nrow(grid_basis_for_b0),
                                                     FUN=temp_function)));
      grid_upper_this_b <- grid_estimated_this_b + 1.96*grid_standard_error_this_b;
      grid_lower_this_b <- grid_estimated_this_b - 1.96*grid_standard_error_this_b;
      grid_fitted_coefficients[[1+i]] <- data.frame(estimate = grid_estimated_this_b,
  ...
  return(list(fitted_coefficients=fitted_coefficients,
              time_grid=time_grid,
