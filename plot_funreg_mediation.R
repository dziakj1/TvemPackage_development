...
#' 
#' @param x The funreg_mediation object to be plotted.
...
#' 
plot.funreg_mediation <- function(x,
                                  use_panes=TRUE, 
                                  what_plot=c("pfr",
                                              "coefs",
                                              "tvem")) {
  ...
  if (what=="coefs") {
    ...
    plot(x$original_results$time_grid,
         x$original_results$gamma_int_estimate,
         xlab=expression(t),
         ylab=expression(gamma[int](t)),
         main="Intercept for Mediator");
    plot(x$original_results$time_grid,
         x$original_results$gamma_X_estimate,
         xlab=expression(t),
         ylab=expression(gamma[X](t)),
         main="Treatment on Mediator");
    plot(x$original_results$time_grid,
         x$original_results$alpha_M_estimate,
         xlab=expression(t),
    ...
    est1 <- round(x$original_results$alpha_int_estimate,2); 
    text(x=1.2,y=3,pos=4,est1);
    text(x=1.1,y=2,pos=4,labels=paste("Indirect effect:"));
    est2 <- round(x$bootstrap_results$beta_boot_estimate,2); 
    text(x=1.2,y=1,pos=4,labels=est2);
  }
  if (what=="tvem") {
    plot.tvem(x$original_results$tvem_XM_details);
  }
}